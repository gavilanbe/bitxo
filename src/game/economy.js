...
function happyDecayRate(p){ return ratePerMs(6) * (1 - 0.1*G.up.juguete) * (p.trait==='JUGUETON'?0.8:1) * weatherHappyMult(p) * (G.relics && G.relics.corona?0.9:1) * (G.toys && G.toys.cometa && WEATHER.kind==='wind' ? 0.65 : 1) * (p.sick?1.5:1) * ((p.zone||'prado')==='parque'?0.8:1) * (typeof decorHappyMult==='function' ? decorHappyMult(p.zone||'prado') : 1); }
function petName(p){ return p.nick || (p.form==='grimo' ? 'GRIMO' : (p.stage===STAGES.EGG ? 'HUEVO' : LINES[p.line].names[p.form||'babyA'])); }
function sleepRegen(p){ return R_SLEEPREGEN * (1 + 0.15*G.up.cama) * (p.trait==='DORMILON'?1.3:1) * (p.sick?0.6:1) * (typeof decorSleepMult==='function' ? decorSleepMult(p.zone||'prado') : 1); }
...
function petRate(p){
  ...
  if(typeof bellezaMotaMult==='function') r *= bellezaMotaMult(p.zone||'prado');
  if(p.sick) r *= 0.5;
  if(p.sleeping) r *= 0.3;
  return r;
}
